<template>
  <view class="page">
    <view class="hero">
      <view class="eyebrow">Excellent Mall</view>
      <view class="hero-title">健康商城与本地生活，装进一个轻量客户端。</view>
      <view class="hero-desc">
        先打通登录、首页、套餐、订单和个人中心，再逐步迁移团队、邀请、佣金和本地生活。
      </view>
    </view>

    <view class="tabs">
      <view
        class="tab"
        :class="{ active: activeTab === 'login' }"
        @click="activeTab = 'login'"
      >
        登录
      </view>
      <view
        class="tab"
        :class="{ active: activeTab === 'register' }"
        @click="activeTab = 'register'"
      >
        注册
      </view>
    </view>

    <view class="card" v-if="activeTab === 'login'">
      <view class="section-title">账号登录</view>
      <input v-model="loginForm.phone" class="input" type="number" placeholder="请输入手机号" />
      <input v-model="loginForm.password" class="input" password placeholder="请输入密码" />
      <button class="primary-btn" @click="handleLogin">立即登录</button>
    </view>

    <view class="card" v-else>
      <view class="section-title">注册并进入</view>
      <input v-model="registerForm.phone" class="input" type="number" placeholder="请输入手机号" />
      <input v-model="registerForm.nickname" class="input" placeholder="请输入昵称" />
      <input v-model="registerForm.password" class="input" password placeholder="请输入密码" />
      <input v-model="registerForm.invite_code" class="input" placeholder="选填邀请码" />
      <button class="primary-btn" @click="handleRegister">注册并进入</button>
    </view>

    <view class="tip-card">
      <view class="tip-title">当前演示账号</view>
      <view class="tip-row">手机号：18800000000</view>
      <view class="tip-row">密码：Admin@123</view>
    </view>
  </view>
</template>

<script setup>
import { reactive, ref } from 'vue'
import { onLoad, onShow } from '@dcloudio/uni-app'

import { authApi } from '../../api/modules'
import { getToken, setToken, setUserCache } from '../../utils/auth'

const activeTab = ref('login')
const loginForm = reactive({
  phone: '18800000000',
  password: 'Admin@123'
})
const registerForm = reactive({
  phone: '',
  nickname: '',
  password: '',
  invite_code: ''
})

function saveAuth(data) {
  setToken(data.access_token)
  setUserCache(data.user)
}

function toHome() {
  uni.switchTab({ url: '/pages/home/index' })
}

async function handleLogin() {
  if (!loginForm.phone || !loginForm.password) {
    uni.showToast({ title: '请填写手机号和密码', icon: 'none' })
    return
  }
  const data = await authApi.login(loginForm)
  saveAuth(data)
  uni.showToast({ title: '登录成功', icon: 'success' })
  toHome()
}

async function handleRegister() {
  if (!registerForm.phone || !registerForm.nickname || !registerForm.password) {
    uni.showToast({ title: '请填写完整注册信息', icon: 'none' })
    return
  }
  const data = await authApi.register({
    ...registerForm,
    invite_code: registerForm.invite_code || null
  })
  saveAuth(data)
  uni.showToast({ title: '注册成功', icon: 'success' })
  toHome()
}

onLoad((options) => {
  if (options?.invite_code) {
    activeTab.value = 'register'
    registerForm.invite_code = String(options.invite_code)
  }
})

onShow(() => {
  if (getToken()) {
    toHome()
  }
})
</script>

<style scoped>
.page {
  min-height: 100vh;
  padding: 40rpx 32rpx;
  background: linear-gradient(180deg, #e8f1ff 0%, #f5f7fb 42%, #f5f7fb 100%);
}

.hero {
  margin-bottom: 32rpx;
}

.eyebrow {
  display: inline-flex;
  align-items: center;
  height: 48rpx;
  padding: 0 20rpx;
  border-radius: 999rpx;
  background: rgba(13, 110, 253, 0.12);
  color: #0d6efd;
  font-size: 24rpx;
  margin-bottom: 20rpx;
}

.hero-title {
  font-size: 52rpx;
  line-height: 1.25;
  font-weight: 700;
  color: #111827;
  margin-bottom: 16rpx;
}

.hero-desc {
  font-size: 28rpx;
  color: #4b5563;
  line-height: 1.7;
}

.tabs {
  display: flex;
  background: #ffffff;
  border-radius: 24rpx;
  padding: 8rpx;
  margin-bottom: 24rpx;
  box-shadow: 0 18rpx 40rpx rgba(15, 23, 42, 0.06);
}

.tab {
  flex: 1;
  text-align: center;
  font-size: 28rpx;
  color: #6b7280;
  padding: 18rpx 0;
  border-radius: 18rpx;
}

.tab.active {
  background: #0d6efd;
  color: #ffffff;
}

.card {
  background: #ffffff;
  border-radius: 24rpx;
  padding: 32rpx;
  box-shadow: 0 18rpx 40rpx rgba(15, 23, 42, 0.06);
  margin-bottom: 24rpx;
}

.section-title {
  font-size: 40rpx;
  font-weight: 600;
  margin-bottom: 24rpx;
}

.input {
  width: 100%;
  height: 88rpx;
  background: #f5f7fb;
  border-radius: 18rpx;
  padding: 0 24rpx;
  font-size: 28rpx;
  margin-bottom: 20rpx;
  box-sizing: border-box;
}

.primary-btn {
  height: 88rpx;
  line-height: 88rpx;
  border-radius: 18rpx;
  background: #0d6efd;
  color: #ffffff;
  font-size: 30rpx;
}

.tip-card {
  background: #111827;
  color: #ffffff;
  border-radius: 24rpx;
  padding: 28rpx 32rpx;
}

.tip-title {
  font-size: 28rpx;
  font-weight: 600;
  margin-bottom: 12rpx;
}

.tip-row {
  font-size: 28rpx;
  color: rgba(255, 255, 255, 0.8);
  line-height: 1.6;
}
</style>
