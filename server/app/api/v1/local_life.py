...
def merchants(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return {'code': 0, 'message': 'success', 'data': LocalLifeServiceLayer.list_merchants(db)}
...
def services(
    ...
    return {'code': 0, 'message': 'success', 'data': LocalLifeServiceLayer.list_services(db, merchant_id)}
...
def merchant_detail(merchant_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return {'code': 0, 'message': 'success', 'data': LocalLifeServiceLayer.get_merchant(db, merchant_id)}
...
def merchant_stores(merchant_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return {'code': 0, 'message': 'success', 'data': LocalLifeServiceLayer.list_stores(db, merchant_id)}
...
def service_detail(service_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return {'code': 0, 'message': 'success', 'data': LocalLifeServiceLayer.get_service(db, service_id)}


@app_router.get('/orders')
def my_local_life_orders(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return {'code': 0, 'message': 'success', 'data': LocalLifeServiceLayer.my_orders(db, current_user.id)}
...
def local_life_order_detail(order_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return {'code': 0, 'message': 'success', 'data': LocalLifeServiceLayer.get_order_detail(db, current_user.id, order_id)}
